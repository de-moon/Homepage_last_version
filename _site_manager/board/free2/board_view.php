...
$stmt->bindColumn(25, $col_category_seq);
$stmt->bindColumn(26, $col_category_subject);
	...
	<div id="content">
		...
		<div class="table_view_type01">
			...
			<table>
				...
				<tr <?php if ( !$BOARD_CATEGORY ) echo "style='display:none;'"; ?>>
					<th>분류</th>
					<td class="tal">
						<?php
						If ($BOARD_CATEGORY) {
							...
							$var_SQL .= "		, ( SELECT SUBJECT FROM ".$BOARD_TABLENAME."_CATEGORY WHERE CATEGORY_SEQ = _ID) AS SUBJECT  ";
							$var_SQL .= " FROM (  ";
							$var_SQL .= "		SELECT  ";
							$var_SQL .= "			@R AS _ID  ";
							...
							$var_SQL .= "	FROM ( SELECT @R :=  ?, @L := 0 ) VARS, ".$BOARD_TABLENAME."_CATEGORY H ";
							$var_SQL .= "	WHERE @R IS NOT NULL AND H.DEL_YN = 'N' ";
							$var_SQL .= "	ORDER BY LEVEL DESC  ";
							$var_SQL .= "	) QI; ";

							$stmt = $pdo->prepare($var_SQL);
							$param = null;
							$param[] = $col_category_seq;
							$arrayStart = 1;
							for ($i = 0; $i < count($param); $i++) {
								$stmt->bindParam($arrayStart, $param[$i]);
								$arrayStart++;
							...
							$totalc = $stmt->rowCount();
							$col_category_subject = "";
							for ($k = 0; $k < $totalc; $k++) {
								$bcrow = $stmt->fetch(PDO::FETCH_OBJ);
								$col_category_subject .= $bcrow->SUBJECT;
								$col_category_subject .= " > ";
							}
							$col_category_subject = substr($col_category_subject, 0, -3);
							$stmt->closeCursor();

							echo $col_category_subject ;
						}
						?>
					</td>
				</tr>
