		...
		<div class="table_view_type01">
			...
			<table>
				...
				<tr <?php if ( !$BOARD_CATEGORY ) echo "style='display:none;'"; ?>>
					<th>분류</th>
					<td class="tal">
						<?php for ($i = 1; $i <= $BOARD_CATEGORY_DEPTH; $i++) { ?>
						<select name="category_seq+<?=$i?>" onchange="sc.get(this)" depth="<?=$i?>" head="선택"></select>
						<?php } ?>
						<script type="text/javascript" src="<?=$COMMON_PATH?>/js/SelectCategory.js"></script>
						<script type="text/javascript">
						<!--
							var sc = new SelectCategory();
							sc.init("<?=$search_division?>", "<?=$search_language_type ?>", document.frm_ins.category_seq);
						//-->
						</script>
					</td>
				</tr>
				...
				<tr <?php if (!$BOARD_ETC_1) { ?>style="display:none;"<?php } ?>>
					<th>기타</th>
					<td class="tal"><input type="text" id="" name="etc_1" value="<?=$col_etc_1 ?>" maxlength="2000" style="width:50%" /></td>
				</tr>
