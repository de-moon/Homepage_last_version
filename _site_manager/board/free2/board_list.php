...
$search_category			= $mod->trans3($_REQUEST, "search_category", "0");
...
if ($search_category != "0") $setParams[] = "search_category=".$search_category;
...
if ($search_category > 0) {
	$var_SQL = "SELECT d.CATEGORY_SEQ";
	$var_SQL .= " FROM (SELECT fnGetCategory2() AS CATEGORY_SEQ, @LEVEL_ AS LEVEL1";
	$var_SQL .= " FROM (SELECT @start_with:=?, @CATEGORY_SEQ:=@start_with, @LEVEL_:=0) vars"; 
	$var_SQL .= " JOIN ".$BOARD_TABLENAME."_CATEGORY WHERE @CATEGORY_SEQ IS NOT NULL"; 
	$var_SQL .= ") func JOIN ".$BOARD_TABLENAME."_CATEGORY d";
	$var_SQL .= " ON func.CATEGORY_SEQ = d.CATEGORY_SEQ";

	$stmt = $pdo->prepare($var_SQL);
	$param2[] = $search_category;
	$arrayStart = 1;
	for ($i = 0; $i < count($param2); $i++) {
		$stmt->bindParam($arrayStart, $param2[$i]);
		$arrayStart++;
	}
	...
	$cm = join(",",$ct);
	$var_AddSQL .= " AND A.CATEGORY_SEQ IN (".$cm.")";
}
...
$var_SQL .= " , A.CATEGORY_SEQ ";
$var_SQL .= " , ( SELECT SUBJECT FROM ".$BOARD_TABLENAME."_CATEGORY B WHERE A.CATEGORY_SEQ = B.CATEGORY_SEQ) AS CATEGORY_SUBJECT ";
	...
	<div id="content">
		...
		<div class="search_type01">
			...
			<div class="col line">
				...
				<div class="select_set" <?php if ( !$BOARD_CATEGORY ) echo "style='display:none;'"; ?>>
					<strong>분류</strong>
					<?php
					for ($i = 1; $i <= $BOARD_CATEGORY_DEPTH; $i++) {
					?>
					<select name="search_category+<?=$i?>" onchange="sc.get(this)" depth="<?=$i?>" head="선택"></select>
					<?php
					}
					?>
				</div>
				<script type="text/javascript" src="<?=$COMMON_PATH?>/js/SelectCategory.js"></script>
				<script type="text/javascript">
				<!--
					var sc = new SelectCategory();
					sc.init("<?=$search_division?>", "<?=$search_language_type ?>", document.frm_search.search_category);
				//-->
				</script>
		...
		<div class="table_list_type01">
			...
			<div class="table_top">
				<div class="left">
					...
					<?php If($BOARD_CATEGORY_MANAGE){ ?><a href="board_category_list.php?<?=$setParams ?>" class="button white">카테고리 관리</a><?php } ?>
				</div>
				<div class="right">
			...
			<table>
				<colgroup>
					<col width="15px" />
					<col width="78px" />
					<?php if ( $BOARD_CATEGORY ) echo "<col width='150px' />"; ?>
					<col width="" />
					<col width="100px" />
					<col width="100px" />
					<col width="120px" />
				...
				<tr>
					...
					<td>공지</td>
					<?php if ( $BOARD_CATEGORY ) echo "<td>".$row['CATEGORY_SUBJECT']."</td>"; ?>
