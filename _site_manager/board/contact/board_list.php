...
if ($search_keyword != "" && $search_field != "" ) {
	if ($search_field == "subject") {
		...
		$var_AddSQL .= " AND (A.ETC_1 LIKE CONCAT('%',:keyword,'%') or A.ETC_3 LIKE CONCAT('%',:keyword,'%'))";
		$param[] = array(":keyword" => $search_keyword);
	} else if ($search_field == "ETC_3"){
		
		$var_AddSQL .= " AND A.ETC_3 LIKE CONCAT('%',:keyword,'%')";
		$param[] = array(":keyword" => $search_keyword);
	}
			...
			<div class="col">
				<div class="select_set">
					...
					<select title="검색키워드" name="search_field">
						<option value="all" <?php if($search_field=="all"){?>selected<?php }?>>전체</option>
						<!--option value="subject"		<?php if($search_field == "subject"){?>selected="selected"<?php }?>>제목</option-->
						<option value="ETC_1"			<?php if($search_field == "ETC_1"){?>selected="selected"<?php }?>>소속</option>
						<option value="ETC_3"			<?php if($search_field == "ETC_3"){?>selected="selected"<?php }?>>이름</option>
						<!--option value="ETC_6"			<?php if($search_field == "ETC_6"){?>selected="selected"<?php }?>>이메일</option-->
		...
		<div class="table_list_type01">
			...
			<table>
				<colgroup>
					<col width="15px" />
					<col width="78px" />
					<?php if ( sizeof($BOARD_CATEGORY) > 0 ) echo "<col width='120px' />"; ?>
					<col width="" />
					<col width="" />
					<col width="" />
					<col width="" />
				...
				<thead>
					<tr>
						...
						<th>
							이름
							<a href="#none" onclick="order_change('ETC_3', 'DESC');" <?php if($search_order_target == "ETC_3" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_3', 'ASC');" <?php if($search_order_target == "ETC_3" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th>
						<th>
							소속
							<a href="#none" onclick="order_change('ETC_1', 'DESC');" <?php if($search_order_target == "ETC_1" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_1', 'ASC');" <?php if($search_order_target == "ETC_1" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th>
						...
						<!--th>
							부서
							<a href="#none" onclick="order_change('ETC_2', 'DESC');" <?php if($search_order_target == "ETC_2" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_2', 'ASC');" <?php if($search_order_target == "ETC_2" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th-->
						
						<!--th>
							직책
							<a href="#none" onclick="order_change('ETC_4', 'DESC');" <?php if($search_order_target == "ETC_4" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_4', 'ASC');" <?php if($search_order_target == "ETC_4" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th-->
						<th>
							연락처
							<a href="#none" onclick="order_change('ETC_5', 'DESC');" <?php if($search_order_target == "ETC_5" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_5', 'ASC');" <?php if($search_order_target == "ETC_5" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th>
						<th>
							이메일
							<a href="#none" onclick="order_change('ETC_6', 'DESC');" <?php if($search_order_target == "ETC_6" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('ETC_6', 'ASC');" <?php if($search_order_target == "ETC_6" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th>
						<th>
							등록일
				...
				<tr>
					...
					<td><?=$row['ETC_2'] ?></td>
					<td><a href="board_view.php?<?=$setParams ?>&page=<?=$var_Page ?>&num=<?=$row['BOARD_SEQ']?>"><strong><?=$row['ETC_3'] ?></strong></a></td>
					<td><?=$row['ETC_1'] ?></td>
					<td><?=$row['ETC_4'] ?></td>
					<td><?=$row['ETC_5'] ?></td>
					<td><?=$row['ETC_6'] ?></td>
