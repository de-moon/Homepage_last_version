...
// ----------------------------------------------------------------------------------------------------
	...
	$var_returl			= $mod->trans3($_POST, "returl", "");
	$ins_id				= $mod->trans3($_POST, "ins_id", "");
	$ins_password	= $mod->trans3($_POST, "ins_password", "");
...
// ####################################################################################################
	...
	if ($total > 0) {
		if($col_password != $ins_password) {
			...
			$_SESSION['MALGUM_USER_SEQ'] = $col_user_seq;
			$_SESSION['MALGUM_USER_ID'] = $col_user_id;
			$_SESSION['MALGUM_USER_NAME'] = $col_user_name;
			$_SESSION['MALGUM_EMAIL'] = $col_email;
			$_SESSION['MALGUM_AUTHORITY'] = $col_authority;
			$_SESSION['MALGUM_AUTHORITY_DETAIL'] = $col_authority_detail;
			$_SESSION['MALGUM_LOGIN_IP'] = $_SERVER['REMOTE_ADDR'];
