...
// 이전글
$var_SQL = " SELECT ";
$var_SQL .= "   A.POPUP_SEQ, A.SUBJECT ";
$var_SQL .= " FROM ".$BOARD_TABLENAME." A ";
$var_SQL .= $var_AddSQL;
$var_SQL .= " AND A.POPUP_SEQ < $col_popup_seq ";
$var_SQL .= " ORDER BY A.POPUP_SEQ DESC LIMIT 1 ";
...
$var_SQL .= $var_AddSQL;
$var_SQL .= " AND A.POPUP_SEQ > $col_popup_seq ";
$var_SQL .= " ORDER BY A.POPUP_SEQ LIMIT 1 ";
		...
		<div class="table_view_type01">
			...
			<table>
				...
				<tr <?php if ( sizeof($BOARD_CATEGORY) < 1 ) echo "style='display:none;'"; ?>>
					<th>분류</th>
					<td class="tal"><?=$BOARD_CATEGORY[$col_category] ?></td>
				</tr>
				<tr>
					<th>노출 순서</th>
					<td class="tal"><?=$col_order_num ?></td>
				</tr>
				<tr>
					<th>노출 기간</th>
					<td class="tal"><?=date("Y-m-d", strtotime($col_start_date)) ?> ~ <?=date("Y-m-d", strtotime($col_end_date)) ?></td>
				</tr>
				<tr>
					<th>제목</th>
					<td class="tal"><?=$col_subject ?></td>
				</tr>
				<tr <?php if (!$BOARD_INPUT_TYPE) { ?>style="display:none;"<?php } ?>>
					<th>팝업내용 유형</th>
					<td class="tal"><?= ( $col_input_type == "E" ) ? "에디터" : "이미지" ?></td>
				</tr>
				<tr <?php if ( $col_input_type == "I" ) { ?>style="display:none;"<?php } ?>>
					<th>내용</th>
					<td class="tal"><?=$col_content ?></td>
				</tr>
				<tr <?php if ( $col_input_type == "E" ) { ?>style="display:none;"<?php } ?>>
					<th>이미지</th>
					<td class="tal">
						<?php
				...
				<tr>
					<th>링크</th>
					<td class="tal"><?=$col_link_url ?></td>
				</tr>
				<tr <?php if (!$BOARD_MOBILE_LINK) { ?>style="display:none;"<?php } ?>>
					<th>모바일 링크</th>
					<td class="tal"><?=$col_link_url_mobile ?></td>
				</tr>
				<tr>
					<th>링크 타겟</th>
					<td class="tal"><?=$col_link_target ?></td>
				</tr>
				<tr>
					<th>등록일</th>
