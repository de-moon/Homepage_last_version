...
// 검색조건
$var_AddSQL		= " WHERE A.USER_DIVISION = 9 AND A.USER_STATE = 1 AND A.USER_ID <> 'MALGUM' ";
...
if ($search_keyword != "" && $search_field != "" ) {
	if ($search_field == "user_id") {
		...
		$var_AddSQL .= " AND A.EMAIL LIKE CONCAT('%',:keyword,'%')";
		$param[] = array(":keyword" => $search_keyword);
	}else{
		$var_AddSQL .= " AND A.USER_NAME LIKE CONCAT('%',:keyword,'%') or A.USER_ID LIKE CONCAT('%',:keyword,'%')";
		$param[] = array(":keyword" => $search_keyword);
	}
}

if ($search_authority != "") {
	$var_AddSQL .= " AND B.AUTHORITY = :authority ";
	$param[] = array(":authority" => $search_authority);
}
...
// 총 레코드 수 / 현재 글번호
$cntq = "SELECT COUNT(*) cnt FROM ".$BOARD_TABLENAME . " A INNER JOIN TB_ADMIN B ON A.USER_SEQ = B.USER_SEQ {$var_AddSQL} ";
...
$var_SQL .= " , C.ETC_1 ";
$var_SQL .= " FROM ".$BOARD_TABLENAME . " A INNER JOIN TB_ADMIN B ON A.USER_SEQ = B.USER_SEQ ";
		...
		<div class="search_type01">
			...
			<div class="col line">
				<div class="select_set">
					<strong>권한</strong>
					<select title="권한" name="search_authority" onchange="search_page();">
						<option value="">전체</option>
						<option value="SUPER"	<?php if($search_authority == "SUPER"){?>selected="selected"<?php } ?>>최고관리자</option>
						<option value="PART"	<?php if($search_authority == "PART") {?>selected="selected"<?php } ?>>부분관리자</option>
					</select>
				</div>
				<!--div class="select_set">
			...
			<div class="col">
				<div class="select_set">
					...
					<select title="검색키워드" name="search_field">
						...
						<option value="email"	<?php if($search_field == "email"){?>selected="selected"<?php } ?>>이메일</option>
					</select>
			...
			<table>
				...
				<thead>
					<tr>
						...
						<th>
							권한
							<a href="#none" onclick="order_change('AUTHORITY', 'DESC');" <?php if($search_order_target == "AUTHORITY" && $search_order_action == "DESC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▲</a>
							<a href="#none" onclick="order_change('AUTHORITY', 'ASC');" <?php if($search_order_target == "AUTHORITY" && $search_order_action == "ASC") {?>class="order_select"<?php } else { ?>class="order_none"<?php } ?>>▼</a>
						</th>
						<th>
							아이디
				...
				<tr>
					...
					<td><?=$num--;?></td>
					<td><?=$BOARD_ADMIN_ARR[$row['AUTHORITY']]?></td>
					<td><a href="board_view.php?<?=$setParams ?>&page=<?=$var_Page ?>&num=<?=$row['USER_SEQ']?>"><strong><?=$row['USER_ID']?></strong></a></td>
					<td><a href="board_view.php?<?=$setParams ?>&page=<?=$var_Page ?>&num=<?=$row['USER_SEQ']?>"><strong><?=$row['USER_NAME']?></strong></a></td>
					<!--td><?=$row['TELEPHONE']?></td-->
					<td><?=$row['CELLPHONE']?></td>
					<td><?=$row['EMAIL']?></td>
					<td><?=$BOARD_RECMAIL[$row['ETC_1']]?></td>
